      ...
      <div className="flex flex-col items-center gap-6 text-center max-w-sm mx-auto">
        ...
        <div>
          ...
          <h2 className="text-3xl font-black tracking-tight"
            style={{ background: "linear-gradient(135deg,#e8c86a,#D4AF37)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            YOU HAVE FOUND
          </h2>
          <h2 className="text-3xl font-black tracking-tight"
            style={{ background: "linear-gradient(135deg,#D4AF37,#C5A059)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
            YOUR GOAT!
          </h2>
        </div>

        <div className="avatar-spin-in flex flex-col items-center gap-4 w-full rounded-2xl p-6"
          style={{ background: "rgba(255,255,255,0.06)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", border: "2px solid #D4AF37", boxShadow: "0 0 60px rgba(212,175,55,0.4), 0 0 120px rgba(212,175,55,0.1)" }}>
          <AvatarCircle player={winner} size={120} />
          <div>
            <p className="text-white font-black text-2xl">{winner.name}</p>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, marginTop: 2 }}>{winner.flag} {winner.country}</p>
          </div>
        </div>

        <p style={{ color: "rgba(255,255,255,0.55)", fontSize: 13 }}>Save your pick and see how the world voted!</p>

        <button onClick={() => setStep("form")} className="btn-gold w-full text-lg" style={{ padding: "14px 0", borderRadius: 50 }}>
      ...
      <div className="w-full max-w-sm mx-auto">
        ...
        <form onSubmit={handleFormSubmit} className="flex flex-col gap-3">
          <input type="text" placeholder="Full Name" value={name} onChange={(e) => setName(e.target.value)} required style={inputStyle}
            onFocus={(e) => (e.target.style.borderColor = "#D4AF37")} onBlur={(e) => (e.target.style.borderColor = "rgba(212,175,55,0.3)")} />
          ...
          {error && (
            <p className="text-sm text-center rounded-xl py-2" style={{ color: "#f87171", background: "rgba(239,68,68,0.1)" }}>{error}</p>
          )}
    ...
    <div className="w-full max-w-sm mx-auto text-center">
      ...
      {devOtpHint && (
        <div className="mb-4 rounded-xl p-3 text-left" style={{ background: "rgba(212,175,55,0.12)", border: "1px solid rgba(212,175,55,0.4)" }}>
          <p className="text-xs font-bold mb-1" style={{ color: "#D4AF37" }}>&#x26A0;&#xFE0F; Email not delivered (Resend trial mode)</p>
      ...
      <form onSubmit={handleOTPSubmit} className="flex flex-col gap-3">
        <input type="text" inputMode="numeric" maxLength={6} placeholder="000000" value={otp}
          ...
          style={{ ...inputStyle, fontSize: 32, fontWeight: 900, letterSpacing: "1rem", textAlign: "center", padding: "16px", color: "#D4AF37", border: "2px solid rgba(212,175,55,0.3)" }}
          onFocus={(e) => (e.target.style.borderColor = "#D4AF37")} onBlur={(e) => (e.target.style.borderColor = "rgba(212,175,55,0.3)")} />
        ...
        <button type="submit" disabled={loading || otp.length !== 6} className="btn-gold w-full"
          ...
          {loading ? "Verifying..." : "Verify & Enter Leaderboard"}
        </button>
      ...
      <p className="text-xs mt-4" style={{ color: "rgba(255,255,255,0.2)" }}>
        Valid for 10 minutes &middot; Check your spam folder
      </p>
    </div>
  );
}
